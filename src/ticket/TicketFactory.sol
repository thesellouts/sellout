...
contract TicketFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function createTicketProxy(address initialOwner) public onlyBoxOffice returns (address) {
        bytes memory initData = abi.encodeWithSelector(
            ITicket.initialize.selector,
            initialOwner,
            version
        );
        ERC1967Proxy proxy = new ERC1967Proxy(ticketImplementation, initData);
        address ticketProxyAddress = address(proxy);
        deployedTickets.push(ticketProxyAddress);
        emit TicketProxyCreated(ticketProxyAddress);
        return ticketProxyAddress;
    }
